...
async def fetch_exness_balance_direct() -> float | None:
    ...
    try:
        ...
        if not mt5.initialize():
            return None
        # Try login — may return False if terminal is already logged in (that's ok)
...
async def sync_account_balance() -> float:
    ...
    if balance:
        ...
        source = "bridge" if settings.use_mt5_bridge else "MT5 direct"
        logger.success(f"Account synced from {source}: ${balance:.2f}")
        return balance
