...
EMA_SPREAD_THRESHOLD = 0.08      # min EMA spread % (was 0.15 — too tight for gold at $4700+)
STOP_MULT            = 2.5       # SL = ATR × 2.5
RR_RATIO             = 3.0       # TP = SL × 2.5
COOLDOWN_BARS        = 6        # bars between signals
MIN_SCORE            = 40        # normal signal threshold (was 55 — yfinance vol=0 removes up to 15pts)
ELITE_SCORE          = 75        # elite signal threshold (was 85)
MIN_PROB             = 65        # min win probability % (was 68)
TRAIL_TRIGGER        = 1.5       # activate trail after 1.5× ATR profit
TRAIL_DISTANCE       = 1.0       # trail distance = 1.0× ATR
MAX_SIGNALS_PER_DAY  = 10
MAX_RSI_BUY          = 78
MIN_RSI_SELL         = 30
VOL_MIN_NORMAL       = 0.0        # yfinance returns vol=0 for gold futures — skip filter
VOL_MIN_ELITE        = 0.0        # same reason
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    raw_ratio = volume / avg_vol.replace(0, np.nan)
    # yfinance gold futures often returns 0 volume — treat as neutral (1.0) not zero
    df["vol_ratio"] = raw_ratio.fillna(1.0).replace(0.0, 1.0)
...
def detect_breakout(df: pd.DataFrame, idx: int) -> Optional[SniperSignal]:
    ...
    adx = float(last.get("adx", 0)) if "adx" in last.index else 0.0

    # BUY breakout: price closes above 20-bar high with volume + ADX trend
    if (price > high20 and vol_ratio > 1.2 and
            rsi > 52 and rsi < 75 and bull and adx > 18):
        sl = price - atr * 2.0
        tp = price + atr * 6.0
        logger.info(
            f"🔥 BREAKOUT BUY: ${price:,.2f} > 20H ${high20:,.2f} | "
            f"Vol={vol_ratio:.1f}x RSI={rsi:.0f} ADX={adx:.0f}"
        )
        return SniperSignal(
    ...
    # SELL breakout: price closes below 20-bar low with volume + ADX trend
    if (price < low20 and vol_ratio > 1.2 and
            rsi < 48 and rsi > 22 and bear and adx > 18):
        sl = price + atr * 2.0
        tp = price - atr * 6.0
        logger.info(
...
def detect_momentum(df: pd.DataFrame, idx: int) -> Optional[SniperSignal]:
    ...
    # ADX — require real trend strength, not sideways noise
    tr = pd.concat([
        df["high"] - df["low"],
    ...
    up_m = df["high"].diff().clip(lower=0)
    dn_m = (-df["low"].diff()).clip(lower=0)
    pdi  = 100 * (up_m.ewm(14, adjust=False).mean() / (tr_s + 1e-9))
    mdi  = 100 * (dn_m.ewm(14, adjust=False).mean() / (tr_s + 1e-9))
    dx   = 100 * (abs(pdi - mdi) / (pdi + mdi + 1e-9))
    adx  = float(dx.ewm(14, adjust=False).mean().iloc[idx])

    # 5-bar momentum (stronger confirmation than 3-bar)
    mom_up   = price > float(df["close"].iloc[idx-5])
    mom_down = price < float(df["close"].iloc[idx-5])

    # Volume — require above average, not just 80%
    vol_avg = float(df["volume"].iloc[idx-20:idx].mean())
    ...
    _session = "london" if 6 <= _now.hour <= 15 else "ny" if 12 <= _now.hour <= 20 else "asian"

    if (bull and mom_up and 48 < rsi < 68 and vol_ok and
            adx > 20 and bull_candle):
        conditions = {
            "bull_trend": bull, "adx_strong": adx > 20,
            "vol_spike": vol_ok, "rsi_zone": True,
            "momentum_5bar": mom_up, "bull_candle": bull_candle,
            "active_session": _session in ("london", "ny"),
            "atr_normal": True,
        }
        try:
            from bot.nexus_learning_brain import should_block_entry
            blocked, reason = should_block_entry(conditions, _session, _now.hour)
            if blocked:
                logger.info(f"🧠 LEARNING BLOCK (BUY): {reason}")
                return _wait_signal(df)
        except Exception:
            pass
        sl = price - atr * 2.0
        tp = price + atr * 6.0
        logger.info(f"⚡ MOMENTUM BUY: ${price:,.2f} RSI={rsi:.0f} ADX={adx:.0f} 5-bar mom up")
        sig = SniperSignal(
        ...
        sig._conditions = conditions
        return sig

    if (bear and mom_down and 32 < rsi < 52 and vol_ok and
            adx > 20 and bear_candle):
        conditions = {
            "bull_trend": False, "adx_strong": adx > 20,
            "vol_spike": vol_ok, "rsi_zone": True,
            "momentum_5bar": mom_down, "bull_candle": False,
            "active_session": _session in ("london", "ny"),
            "atr_normal": True,
        }
        try:
            ...
            blocked, reason = should_block_entry(conditions, _session, _now.hour)
            if blocked:
                logger.info(f"🧠 LEARNING BLOCK (SELL): {reason}")
                return _wait_signal(df)
        except Exception:
            pass
        sl = price + atr * 2.0
        tp = price - atr * 6.0
        logger.info(f"⚡ MOMENTUM SELL: ${price:,.2f} RSI={rsi:.0f} ADX={adx:.0f} 5-bar mom down")
        sig = SniperSignal(
...
async def run_sniper(bar_index: int = 0) -> Optional[SniperSignal]:
    ...
    # ── Fetch multi-timeframe S/R levels ──────────────
    current_price = float(df.iloc[-1]["close"])
    current_atr   = float(df.iloc[-1]["atr"])
    sr = None
    try:
        from bot.sr_levels import get_sr_levels, sr_score, sr_summary
        sr = await get_sr_levels(current_price, current_atr)
        logger.info(f"📊 S/R: {sr_summary(sr)}")
    except Exception as e:
        logger.debug(f"SR fetch skipped: {e}")
    ...
    if signal.is_actionable:
        # ── S/R filter — block bad entries, boost good ones ──
        if sr:
            try:
                adj, note = sr_score(sr, signal.direction, signal.entry, signal.atr)
                if adj <= -20:
                    # Entry is directly into a wall — skip
                    logger.warning(
                        f"⛔ S/R BLOCK: {signal.direction} rejected — {note}"
                    )
                    return None
                if note:
                    logger.info(f"📊 S/R: {note}  (adj={adj:+d})")
                # Boost probability for entries near key levels
                if adj >= 8:
                    signal = SniperSignal(
                        direction   = signal.direction,
                        signal_type = signal.signal_type,
                        score       = min(100, signal.score + adj),
                        probability = min(90, signal.probability + adj * 0.5),
                        entry       = signal.entry,
                ...
                logger.debug(f"SR score error: {e}")
