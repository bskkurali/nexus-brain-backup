...
OLLAMA_MODEL  = "llama3.2:1b"       # 1B model — 3x faster on CPU VPS than 3.2B
...
async def agent_call(prompt: str, max_tokens: int = 500) -> str:
    """
    Priority chain (Ollama too slow on CPU VPS — Groq is primary):
      1. Groq    (cloud — free, fast, 30 req/min)
      2. Gemini  (cloud — free, rotates Key1 → Key2)
      3. Ollama  (local — fallback only, slow on CPU)
      4. Claude  (cloud — paid, last resort)
    """
    from config.settings import settings

    # ── 0. Ollama (local — primary, no rate limits) ──
    result = await ollama_call(prompt, max_tokens)
    if result:
        return result

    # ── 1. Groq (free cloud, fast) — wait 8s on 429 then retry ──
        ...
        for groq_attempt in range(2):
            try:
                ...
                if "429" in err and groq_attempt == 0:
                    logger.debug("Groq 429 — waiting 8s then retry")
                    await asyncio.sleep(8)
                    continue
                logger.warning(f"Groq failed: {err[:50]}")
                break

    # ── 2. Gemini — rotates Key1 → Key2 ─────────────────────────
    gemini_keys = [k for k in [
        settings.google_ai_api_key,
        settings.google_ai_api_key_2,
    ] if k]
    for i, gkey in enumerate(gemini_keys):
        result = await gemma_call(prompt, gkey, max_tokens)
        if result:
            logger.debug(f"Gemini key{i+1} ✅")
            return result
        logger.debug(
            f"Gemini key{i+1} failed — "
            f"{'trying key2' if i == 0 and len(gemini_keys) > 1 else 'no more keys'}"
        )
...
# ── Aliases (api_key kept for backward compatibility, ignored) ────
