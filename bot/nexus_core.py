...
def risk_agent() -> dict:
    ...
    elif 1290<=m<1380: sess="NEW_YORK";         sess_ok=True;  sess_score=85
...
async def master_brain_cycle() -> dict:
    ...
    # ── Step 3: Master Brain → Claude (1 call) ─────────
...
{{
  ...
  "spawn_agent": null or "describe new agent needed e.g. correlation"
    ...
    try:
        ...
        logger.error(f"Master Brain Claude call: {e}")
        return {"executed":False,"reason":f"Claude error: {str(e)[:60]}"}
