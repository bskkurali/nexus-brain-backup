...
async def main_async(mode: str):
    ...
    # ── NEXUS Master Brain (Claude AI decision maker) ──
    async def nexus_brain_cycle():
        try:
            ...
            if result.get("executed"):
                logger.success(
                    f"🧠 NEXUS BRAIN TRADE: {result.get('direction')} "
                    f"@ ${result.get('entry', 0):,.2f} conf={result.get('confidence', 0)}%"
                )
            else:
                logger.debug(f"🧠 NEXUS BRAIN: {result.get('reason','no trade')}")
        except Exception as e:
            logger.error(f"NEXUS Brain: {e}")
    ...
    # Run first scans staggered — prevents all strategies hitting APIs simultaneously
    await sniper_cycle()
